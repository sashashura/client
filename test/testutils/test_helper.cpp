...
public:
    ...
    bool execute(QDir &rootDir) const override
    {
        ...
        cerr << qPrintable(name) << ", file: " << qPrintable(_fileName) << ", secs: " << _secs << endl;
...
public:
    ...
    bool execute(QDir &rootDir) const override
    {
        ...
        cerr << ".... file: " << qPrintable(_fileName) << ", byte: " << ch << endl;
        if (!f.seek(f.size())) {
...
public:
    ...
    bool execute(QDir &rootDir) const override
    {
        cerr << qPrintable(name) << " '" << qPrintable(_fileName) << "' with "
...
public:
    ...
    bool execute(QDir &rootDir) const override
    {
        cerr << qPrintable(name) << " " << qPrintable(_fileName) << endl;
    ...
    static Command *parse(QStringListIterator &it)
    {
        QString fileName = parseFileName(it);
        if (fileName.isEmpty()) {
            cerr << "Error: invalid directory name for " << qPrintable(name) << " command" << endl;
            return {};
        }
...
const QString MkdirCommand::name = QStringLiteral("mkdir");
...
vector<Command *> parseArguments(QStringListIterator &it)
{
    ...
    while (it.hasNext()) {
        ...
        } else if (option == MkdirCommand::name) {
            if (auto cmd = MkdirCommand::parse(it)) {
                commands.emplace_back(cmd);
            } else {
                return {};
            }
