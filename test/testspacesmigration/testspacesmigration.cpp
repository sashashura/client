...
private slots:
    void test()
    {
        FakeFolder fakeFoler({});
        fakeFoler.setServerOverride([&](QNetworkAccessManager::Operation op, const QNetworkRequest &request, QIODevice *) -> QNetworkReply * {
            QString payloadName;
        ...
        cap[QStringLiteral("migration")] = QVariantMap {
            { QStringLiteral("space_migration"), QVariantMap { { QStringLiteral("enabled"), true }, { QStringLiteral("endpoint"), QStringLiteral("migration/spaces") } } }
        };
        fakeFoler.account()->setCapabilities(cap);


        const QUrl personalUrl(QStringLiteral("https://ocis.owncloud.test/dav/spaces/1284d238-aa92-42ce-bdc4-0b0000009157$534bb038-6f9d-4093-946f-133be61fa4e7"));
        QByteArray folder1Uuid;
        AccountStatePtr newAccountState = AccountState::fromNewAccount(fakeFoler.account());
        {
            ...
            QVERIFY(fakeFoler.account()->capabilities().migration().space_migration.enabled);
            auto *migr = new SpaceMigration(newAccountState, fakeFoler.account()->capabilities().migration().space_migration.endpoint, this);
            QSignalSpy spy(migr, &SpaceMigration::finised);
            migr->start();
            QVERIFY(spy.wait());
            migr->deleteLater();
            ...
            QCOMPARE(folder3->webDavUrl(), QUrl(QStringLiteral("https://ocis.owncloud.test/dav/spaces/1284d238-aa92-42ce-bdc4-0b0000009157$31f599a1-94e1-4dfb-8bc9-841a57066d05")));
            ...
            QCOMPARE(folder4->webDavUrl(), fakeFoler.account()->davUrl());
